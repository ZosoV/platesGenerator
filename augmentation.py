...
def change_light_by_hsv(image, coeff):
...
def brighten(image, brightness_coeff=-1): ##function to brighten the image
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            ...
            image_RGB.append(change_light_by_hsv(img,brightness_coeff_t))
    else:
        if(brightness_coeff==-1):
            brightness_coeff_t=1+ random.uniform(0,1) ## coeff between 1.0 and 1.5
        else:
            brightness_coeff_t=1+ brightness_coeff ## coeff between 1.0 and 2.0
        image_RGB= change_light_by_hsv(image,brightness_coeff_t)
    return image_RGB
...
def random_brightness(image,interval = (-1.5,0.5)):
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            ...
            image_RGB.append(change_light_by_hsv(img,random_brightness_coefficient))
            # image_RGB.append(brighten(img,random_brightness_coefficient))
            brighten
    else:
        random_brightness_coefficient = np.random.uniform(interval[0],interval[1]) ## generates value between 0.0 and 2.0
        image_RGB= change_light_by_hsv(image,random_brightness_coefficient)
        # image_RGB= brighten(image,random_brightness_coefficient)
    return image_RGB
